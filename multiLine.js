var multiLine = function multLine(all_data, selector, params) {
  ...
  var svg = d3
    ...
    for (let d=0 ; d<chart_params['group_names'].length; d++){        //if (chart_params['group_names'].length > 1){
  ...
  dots = svg
      ...
      .y(function(d) {
	      return yScale(d.y[group] / (normalize_flag === 0 ? 1 : norm_factors[group]));
	  });

      lines.append("path").attr("class", "line"+selector_class +  " line"+selector_class+group)
      ...
      dots.append("circle")
      .attr("class", "circle"+selector_class + " circle"+selector_class+group)// + group)
      .attr("cx", function(d) {
	      return xScale(d.x);
	  })
      .attr("cy", function(d) {
  ...
  legend
    ...
    yaxis_secondary.selectAll("text").attr("dx", "0.5em").attr("font-color", colors[i1]);
    ...
    //y grid line
	  ...
	  yaxis_secondary.call(g => g.select(".domain").remove());
	    ...
	    d3.selectAll(".circle"+selector_class).style("opacity", function(d2, i2) {
